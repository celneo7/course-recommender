...
def extract_grade_disclosed(text: str) -> dict:
    ...
    gm = re.search(r"[Ff]inal\s*grade[:\s]+([A-F][+-]?)", text)
    if gm:
        result["final_grade"] = gm.group(1)
    # Only flag scores_disclosed for actual exam/component scores like "32/75" or "67/100",
    # not AY years ("AY13/14") or percentage breakdowns ("25%")
    if re.search(r"\b(?:mids?|finals?|practical|exam|quiz|total|lab|ila|tha|oda)[^.\n]{0,30}?(\d+)\s*/\s*(\d+)", text, re.IGNORECASE):
